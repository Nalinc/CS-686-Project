...
\begin{center}
	\begin{tabular}{ |c|c|c| } 
		...
		Recall@1 & 49.5\% & 50.7\% \\ \hline
		Recall@2 & 59.7\% & 68.9\% \\ \hline
		Recall@5 & 76.6\% & 91.3\% \\ \hline
	\end{tabular}
	\captionof{table}{Results for the three algorithms using various recall measures}
\end{center}
...
As mentioned earlier, nBot uses a pre-trained word embeddings model from Associated Press News for training. This model is trained on a Recurrent Neural Netowrk consisting of 4 hidden layers with LSTM cells each having an output dimension of 300. Both the embedded context and response are fed into the same Recurrent Neural Network word-by-word. Inside the recurrent neural network, input context(c) is multiplied with a matrix M to "predict" a response r'. In nBot, input context(c) is a 300-dimensional vector, M is a 300×300 dimensional matrix, and the result is another 300-dimensional vector, which we can be interpreted as a generated response. The matrix M is learned during training. We measure the similarity of the predicted response r' and the actual response r by taking the dot product of these two vectors. A large dot product means the vectors are similar and that the response should receive a high score. We then apply a sigmoid function to convert that score into a probability. Weight initialization as well as the choice of optimizer were critical for training the RNNs. We have used "adam optimizer" with "cosine proximity" as the loss/cost function. Further, the model has used "glorot normal" initializer to set the initial random weights of Keras layers. This implementation is built on top of keras with tensorflow backend.
\\\\
fixed input size issue.
Following is the sample conversation generated by nBot after training for 5000 epochs. These alien words are due to the presence of padding elements in input.
	\begin{tcolorbox}
	\textbf{User:}hi \\
...
\end{itemize}
Most of these requirements are satisfied by the Ubuntu Dialogue Corpus introducted by \cite{lowe2015ubuntu}.
